...
/// Enable Linux TCP_SAVE_SYN on a listening socket so accepted connections
/// can retrieve the client SYN via TCP_SAVED_SYN (no CAP_NET_RAW required).
#[cfg(target_os = "linux")]
fn enable_tcp_save_syn(fd: std::os::unix::io::RawFd) {
    // linux/tcp.h: TCP_SAVE_SYN = 27
    ...
    if rc != 0 {
        log::debug!(
            "TCP_SAVE_SYN enable failed on fd={fd}: {}",
            std::io::Error::last_os_error()
        );
    } else {
        log::info!("TCP_SAVE_SYN enabled on listen fd={fd} (kernel SYN capture)");
    }
}

#[cfg(not(target_os = "linux"))]
fn enable_tcp_save_syn(_fd: std::os::unix::io::RawFd) {}
        ...
        match listener_socket.bind(sock_addr) {
            Ok(()) => {
                ...
                #[cfg(unix)]
                {
                    ...
                    enable_tcp_save_syn(listener.as_raw_fd());
                }
                break Ok(listener.into())
            }
            Err(e) => {
