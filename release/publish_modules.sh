...
#   bash 04-release-github-ci/release/publish_modules.sh analyze ingest
...
crate_for() {
  case "$1" in
    identity) echo gr-module-identity ;;
    brain) echo gr-module-brain ;;
    analyze) echo gr-module-analyze ;;
    ingest) echo gr-module-ingest ;;
    edge) echo gr-module-edge ;;
    probe_assets) echo gr-module-probe-assets ;;
...
for name in "$@"; do
  ...
  so="$(find target/release -maxdepth 1 -name "libgr_module_${name}*.so" -o -name "lib${crate//-/_}.so" 2>/dev/null | head -1)"
  if [[ -z "${so:-}" || ! -f "$so" ]]; then
    so="$(ls -1 target/release/libgr_module_${name}*.so 2>/dev/null | head -1 || true)"
  fi
  if [[ -z "${so:-}" || ! -f "$so" ]]; then
    # rustc often names libgr_module_analyze.so
    so="$(ls -1 target/release/libgr_module_${name//_/-}.so target/release/libgr_${name}.so 2>/dev/null | head -1 || true)"
  fi
  if [[ -z "${so:-}" || ! -f "$so" ]]; then
    so="$(find target/release -maxdepth 1 -name "*.so" | rg -i "module.?${name}|gr.?${name}|gv6.?${name}" | head -1 || true)"
  fi
  if [[ -z "${so:-}" || ! -f "$so" ]]; then
...
if old_path.is_file():
    ...
    mods = list(by.values())
doc.update({
    "product": doc.get("product") or "green-v7",
...
if [[ -n "$OLD_PK" && -n "$OLD_CERT" && -n "$OLD_BUILD_ID" ]]; then
  ...
  cert="$(cargo run -q -p gr-cli -- sign-cert --secret-key keys/ota_ed25519.sk \
    --version "$VERSION" --build-id "${GR_BUILD_ID:-dev}" --release-pubkey "$RELEASE_PK_HEX")"
  RELEASE_ARGS=(--build-id "${GR_BUILD_ID:-dev}" --release-pubkey "$RELEASE_PK_HEX" --release-cert "$cert")
  echo -n "$cert" > "$OUT/release_cert.sig"
  echo -n "$RELEASE_PK_HEX" > "$OUT/release_pubkey.hex"
fi
cargo run -q -p gr-cli -- sign-manifest --manifest "$MANIFEST" \
