/* green-v5 registry.mid — full dynamic packs; auto-split from registry.js */
(function (global) {
  "use strict";
  var C = global.GRCollectors;
  if (!C || !C.register) {
    try { console.warn("[gr] registry.mid: static registry not loaded"); } catch (e) {}
    return;
  }
  if (C.__midLoaded || (C.get && C.get("B16_fast_signals"))) {
    C.__midLoaded = true;
    return;
  }
  ...
  if (typeof midFields !== "function" || typeof enqueue !== "function") {
    try { console.warn("[gr] registry.mid: helpers missing from static registry"); } catch (e2) {}
    return;
  }
  ...
  C.__midLoaded = true;
  C.__midFamilies = { core: true, gpu: true, misc: true };
  try {
    C.midCount = function () {
      return (C.ids() || []).filter(function (id) {
        var d = C.get(id);
        return d && d.schedule === "dynamic";
      }).length;
    };
  } catch (e3) {}
