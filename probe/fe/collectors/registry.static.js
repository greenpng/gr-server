/* green-v5 registry.static — auto-split from registry.js; do not edit by hand */
/**
...
(function (global) {
  ...
  function webglResidualMultiPath(opts) {
    ...
    // P0 method matrix: stall recovery may shrink multipath budget (gecko thrash).
    try {
      var hintCap = global.__GR_MULTIPATH_CAP_HINT__;
      if (hintCap != null && Number(hintCap) >= 0) {
        // 0 = explicit degrade (cpu/audio only) — allow collapse
        if (Number(hintCap) === 0) PATH_CAP = 0;
        else PATH_CAP = Math.max(PATH_FLOOR, Math.min(PATH_CAP, Number(hintCap)));
      }
      // Profile hint for multipath (softgl / webkit_deep / …) when caller used default
      var profHint = global.__GR_PROBE_PROFILE_HINT__;
      if (profHint && String(opts.profile || "default") === "default") {
        opts.profile = String(profHint);
        profile = String(profHint);
      }
    } catch (eHint) {}
  ...
  function scheduleEagerB10xChain(ctx) {
    ...
    // P0: engine-aware order via GRProbeMethodMatrix (webkit prefers webkit_* first).
    var silicon = [
    ...
    try {
      ...
      if (MM && typeof MM.engineB10xOrder === "function") {
        var ordered = MM.engineB10xOrder();
        if (ordered && ordered.length) silicon = ordered;
      } else {
        var eng0 =
          (global.GRCollectors &&
            GRCollectors.__h &&
            typeof GRCollectors.__h.detectEngineFamily === "function" &&
            GRCollectors.__h.detectEngineFamily()) ||
          "unknown";
        eng0 = String(eng0 || "").toLowerCase();
        if (eng0.indexOf("webkit") >= 0) {
          silicon = [
  ...
  function packsFromRoutePlan(routePlan, alreadyKicked) {
    ...
    packs.forEach(function (p) {
      ...
      out.push({
        ...
        pack_id: id,
        priority: p.priority != null ? p.priority : d.priority,
