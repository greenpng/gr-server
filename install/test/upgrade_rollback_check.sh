#!/usr/bin/env bash
# V7 upgrade + version-control contract (runs on public greenpng/install
# GitHub Runners, both archs — free standard runners):
...
bash "$ROOT/release/update_runtime_from_github.sh"
...
if INSTALL_ROOT="$INST" \
  ...
  REQUIRE_SHA=1 \
  GR_ALLOW_UNSIGNED_FIXTURE=1 \
  bash "$ROOT/release/update_runtime_from_github.sh" >/tmp/v7-upgrade-bad.log 2>&1; then
...
if INSTALL_ROOT="$INST" \
  ...
  REQUIRE_SHA=1 \
  GR_ALLOW_UNSIGNED_FIXTURE=1 \
  bash "$ROOT/release/update_runtime_from_github.sh" >/tmp/v7-upgrade-fe-bad.log 2>&1; then
